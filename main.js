...
function renderShadow() {
  const cosY = Math.abs(Math.cos(ry * Math.PI / 180));   // 侧对镜头时投影变窄
  const lean = Math.abs(rx) / 70;                        // 倾斜度 0~1
  const sx = scale * (0.25 + 0.75 * cosY);
  const sy = scale * (1 - 0.3 * lean);
  shadow.style.transform = 'translateY(' + (rx * 0.3) + 'px) scale(' + sx.toFixed(3) + ', ' + sy.toFixed(3) + ')';
  shadow.style.opacity = (0.45 * (1 - 0.35 * lean)).toFixed(2);
}
renderShadow();
...
document.getElementById('aboutClose').addEventListener('click', () => {
  document.getElementById('aboutDialog').open = false;
});

/* ---- 右边按钮 ---- */
const HOME_URL = 'http://squirrellaofang.top/';
